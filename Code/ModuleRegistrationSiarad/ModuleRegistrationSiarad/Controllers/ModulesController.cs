...
namespace ModuleRegistrationSiarad.Controllers
{
    ...
    public class ModulesController : ControllerBase
    {
        ...
        public ActionResult<Module> GetModulesForSpecificYear(String year)
        {
            ...
            var moduleList = _context.module.Where(m => m.year.Equals(year));
            if (moduleList == null)
            {
                return NotFound();
            }
        ...
        public ActionResult<string> GetSpecificModuleForSpecificYear(String year, String mid)
        {
            ...
            var specificModule = _context.module.Where(r => r.year.Equals(year) && r.module_id.Equals(mid));
        ...
        {
            var mod = await _context.module.FirstOrDefaultAsync(m => m.module_id.Equals(module_id) && m.year.Equals(year));
            if(mod != null)
            {
                return BadRequest();
            }
        ...
        {
            var mod = await _context.module.FirstOrDefaultAsync(m => m.module_id.Equals(module.module_id));
            if (mod != null)
            {
                return BadRequest();
            }
        ...
        public async Task<ActionResult<Module>> PostCsvFile(IFormFile test)
        {
            ...
            List<Tuple<String, String>> keySet = new List<Tuple<String, String>>();
            String[] splitCsv = content.ToString().Split("&#£!*");
            //Year = 0
            //Code = 3
            //Name = 5
            //Coordinator = 13
            String year = ""; String module_id = ""; String module_title = ""; String coordinator_id = "";
            for(int i =16; i<splitCsv.Length-1; i++)
            {
                ...
                if (i % 16 == 15)
                {
                    ...
                    if (_context.module.Find(module_id, year)== null){ //Checks for pre-existing data prior to this request
                        if (keySet.Any(r => r.Item1.Equals(year) && r.Item2.Equals(module_id)) == false) //Checks for duplications during this request
                        {
                            _context.Add(module);
                            keySet.Add(new Tuple<String, String>(year, module_id));
                        }
                    }
                    year = "";
                    module_id = "";
...
public async Task<ActionResult> DeleteFromSpecificYear(String year, String module_id)
{
    ...
    var module = await _context.module.FirstOrDefaultAsync(m => m.module_id.Equals(module_id) && m.year.Equals(year));
    if(module==null){
        return NotFound();
    }
    try
...
public async Task<ActionResult> Delete(String id)
{
    ...
    var module = await _context.module.FirstOrDefaultAsync(m => m.module_id.Equals(id));
    if(module == null)
    {
        return NotFound();
    }
...
{
    return _context.module.Any(m => m.module_id.Equals(id));
}
}
}
